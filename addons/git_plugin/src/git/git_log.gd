...
static func execute(item_number: int = 0, request: GitPlugin_CommandRequest = null) -> Array:
	...
	var result
	if item_number > 0:
		result = await GitPlugin_Executor.execute(r'git log --pretty="%H;;;%cd;;;%s " --date=iso ' + str(-item_number), request)
	else:
		result = await GitPlugin_Executor.execute(r'git log --pretty="%H;;;%cd;;;%s " --date=iso', request)
	if result:
