...
static func is_click_left(event: InputEvent, pressed : bool = true) -> bool:
...
##[br][code]event[/code]  事件对象
##[br][code]pressed[/code]  是否按下
static func is_click_right(event: InputEvent, pressed : bool = true) -> bool:
	if event is InputEventMouseButton:
...
##[br]
##[br][code]event[/code]  事件对象
static func is_click_middle(event: InputEvent, pressed : bool = true) -> bool:
	if event is InputEventMouseButton:
		return event.button_index == MOUSE_BUTTON_MIDDLE and event.pressed == pressed
	return false
