...
static func instance_class_scene(_class: Script, callback: Callable = Callable()) -> Node:
	...
	var scene = DataUtil.get_value_or_add(data, _class, func():
...
static var _pool_objects_dict : Dictionary = {}
...
##[br]示例：
##[codeblock]
##var area = NodeUtil.get_pool_object(Area2D)
##var player = NodeUtil.get_pool_object(load("res://role/player/player.tscn"))
##[/codeblock]
##[br]
##[br][b]注意：[/b]尽量不要给这个节点记录到其他特殊的标记，除非你可以在这个节点退出树时清除掉，
##否则因为这些节点重复出现，可能会出现判断错误的情况
static func get_pool_object(type) -> Object:
	var class_object : Variant = ScriptUtil.get_class_object(type)
	var list := Array(_pool_objects_dict.get(class_object, []))
	if list.size() > 0:
		return list.pop_back()
	else:
		var node = type.new()
		node.set_meta("__pool_object", type)
		return node

## 回收对象到池中
static func retrieve_pool_object(object: Object) -> void:
	var class_object : Variant
	if object.has_meta("__pool_object"):
		class_object = object.get_meta("__pool_object")
	else:
		class_object = ScriptUtil.get_class_object(object)
	var list := Array(_pool_objects_dict.get(class_object, []))
	if object is Node:
