...
## 检测到节点的进入
signal detected_enter(node: Node2D)
## 检测到节点的离开
signal detected_exit(node: Node2D)
## 时间结束检测完成
signal finished
...
@export_range(0, 1, 0.001, "or_greater", "hide_slider") var auto_detect_time : float = 0.0:
	set(v):
...
@export var one_shot : bool = false ## 只自动调用一次
@export var detect_area : bool = true: ## 可以检测到 Area2D
	set(v):
		detect_area = v
		if not is_inside_tree():
			return
		_update_conncet_signal()
@export var detect_body : bool = true: ## 可以检测到 PhysicsBody2D 相关的物理节点
	set(v):
		detect_body = v
		if not is_inside_tree():
			return
...
func _detect_enter(node: Node2D, ignore_time_left: bool = false):
	if (
		...
		self.detected_enter.emit(node)

func _detect_exit(node: Node2D):
	if _detected_nodes.has(node):
		_detected_nodes.erase(node)
		self.detected_exit.emit(node)
...
## 检测节点。检测到的对象会以 [signal detected_enter] 信号发出
