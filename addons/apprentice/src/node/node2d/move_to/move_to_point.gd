...
@onready var _to_point : Vector2 = global_position.floor()
...
func _physics_process(delta):
	...
	_update_arrive_status()
	if not _arrive_status:
		# 没有到达位置时，发出移动信号
...
#   自定义
#============================================================
## 到达目标位置条件方法，可以修改这个属性重写条件方法
var arrive_condition : Callable = func(target_position: Vector2):
	return self.global_position.distance_squared_to(_to_point) <= _arrived_squared_distance

func _update_arrive_status():
	_arrive_status = arrive_condition.call(_to_point) 

#  更新移动方向
func _update_move_direction() -> void:
	_last_rot = global_rotation
	_to_dir = global_position.floor().direction_to(_to_point)
...
func to(global_point: Vector2) -> void:
	if global_point != _to_point or not is_physics_processing():
		_time = 0
		_arrive_status = false
		_to_point = global_point.floor()
