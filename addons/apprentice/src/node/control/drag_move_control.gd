...
@export var enabled_parent: bool = true
		...
		if event is InputEventMouseMotion:
			if _clicked_pos != Vector2.INF and Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT):
				...
				if _dragged:
					...
					if diff.length() > 5:
						_dragged = true
						self.dragged.emit(global_position)
			...
			if event.button_index == MOUSE_BUTTON_LEFT:
				if event.pressed:
					...
					if center:
						var center_pos = _clicked_mouse_pos + get_parent().size / 2
						get_parent().global_position = center_pos
				else:
					if _dragged:
						_dragged = false
						self.dropped.emit( global_position )
	)

func get_last_click_pos() -> Vector2:
	return _clicked_pos
