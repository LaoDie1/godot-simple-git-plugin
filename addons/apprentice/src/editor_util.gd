...
static func get_editor_node_map(refresh: bool = false) -> Dictionary:
	if refresh:
		DataUtil.remove_singleton("EditorUtil_get_editor_node_by_class")
...
static func get_scene_data(scene: PackedScene) -> Dictionary:
	...
	_scene_data_cache[scene] = path_to_node_data
...
static func update_file(path: String):
