...
class ProxyScript:
	...
	static func _arg_list(arg_count: int) -> String:
		return ", ".join( 
			range(arg_count).map( func(i): return "arg%d" % i ) 
		)
	...
	static func create_emit_script(
		...
		return DataUtil.get_value_or_add(count_map, arg_num_key, func():
	...
	static func create_no_arg_script(from_signal_arg_num: int) -> GDScript:
		...
		return DataUtil.get_value_or_add(map_data, from_signal_arg_num, func():
			var script = GDScript.new()
...
static func get_argument_num(_signal: Signal) -> int:
	...
	return DataUtil.get_value_or_add(data, _signal, func():
		# 获取这个信号的参数数量
...
##[br]- [code]method[/code]  连接的没有参数的方法
##[br]- [code]object[/code]  代理对象的生命依赖对象
static func get_method_by_sginal_call_no_arg(_signal: Signal, method: Callable, object: Object = null):
...
static func connect_no_arg_method(
	...
	for data in obj.get_signal_list():
		if data['name'] == _signal.get_name():
			if data['args'].size() == 0:
				_signal.connect(method, connect_flag)
				return method
			break
	var callable = get_method_by_sginal_call_no_arg(_signal, method, object)
...
## 断开连接方法
static func disconnect_if(_signal:Signal, method: Callable):
	if _signal.is_connected(method):
		_signal.disconnect(method)


static var _provity_cache_dict: Dictionary = {}

## 根据优先级重新进行排序连接。默认使用这个方法连接的信号的优先级都是 [kbd]0[/kbd]
static func connect_provity(signal_: Signal, method: Callable, flags: int = 0, provity: int = 0):
	if provity != 0:
		#- signal 是对此信号的引用。
		#- callable 是对连接的 Callable 的引用。
		#- flags 是 Object.ConnectFlags 的组合。
		var signal_to_method_dict := _provity_cache_dict.get_or_add(signal_, {}) as Dictionary
		var temp_method_provity: int = 0
		var temp_provity_connects_dict : Dictionary = {}
		var temp_method_group_data_list: Array
		for data in signal_.get_connections():
			temp_method_provity = signal_to_method_dict.get(data["callable"], 0)
			temp_method_group_data_list = temp_provity_connects_dict.get_or_add(temp_method_provity, []) as Array
			temp_method_group_data_list.append(data)
		
		# 有两个以上的连接优先级，则进行排序连接
		temp_method_group_data_list = temp_provity_connects_dict.get_or_add(provity, []) as Array
		temp_method_group_data_list.append({
			"callable": method,
			"flags": flags,
		})
		if temp_provity_connects_dict.size() > 1:
			var provity_list := temp_provity_connects_dict.keys()
			provity_list.sort()  #从小到大排序
			if provity_list.back() == provity: #优先级就是最后末尾的，则不用排序
				signal_.connect(method, flags)
			else:
				# 断开所有
				for data in signal_.get_connections():
					signal_.disconnect(data["callable"])
				# 重新对记录的排序后的数据进行连接
				for p in provity_list:
					temp_method_group_data_list = temp_provity_connects_dict[p] as Array
					for method_data in temp_method_group_data_list:
						signal_.connect(method_data["callable"], method_data["flags"])
		else:
			signal_.connect(method, flags)
		signal_to_method_dict[method] = provity
	else:
		signal_.connect(method, flags)
